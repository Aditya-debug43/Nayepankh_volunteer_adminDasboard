...
exports.refresh = async (req, res, next) => {
  try {
    ...
    // Atomically consume the presented refresh token. findOneAndUpdate matches the user
    // ONLY if this exact (still-valid) token is present, and removes it in the same atomic
    // operation. If two refresh requests race (e.g. multiple tabs reloading at once), only
    // the first matches; the second gets null and is rejected. This replaces the previous
    // read-modify-write rotation, whose lost-update could leave the cookie and DB out of
    // sync and silently log the user out on subsequent refreshes.
    const user = await User.findOneAndUpdate(
      { _id: decoded.sub, 'refreshTokens.token': hashed },
      { $pull: { refreshTokens: { token: hashed } } },
      { new: true }
    );
    if (!user) return res.status(401).json({ message: 'Invalid refresh token.' });

    // Issue and store a freshly rotated refresh token.
    const newRefresh = signRefreshToken(user._id);
    await User.updateOne({ _id: user._id }, { $push: { refreshTokens: { token: hashToken(newRefresh) } } });
